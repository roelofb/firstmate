...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill activates it through `bin/fm-afk-arm.sh` - the daemon's own verified arm, which mirrors the watcher arm's honest `started` / `healthy` / `FAILED` contract and stays attached so a later daemon death re-notifies firstmate - after which the watcher reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.
The daemon validates its supervisor endpoint at startup and refuses loudly instead of dying silently, leaving a durable `state/.subsuper-startup-failed` marker (cleared on the next healthy startup) alongside stderr captured to `state/.supervise-daemon.err`.
Only after that validation succeeds does the daemon publish a `state/.supervise-daemon.ready` marker (removed again on shutdown); the arm reports `started` or `healthy` only when pidfile, lock, live pid, and that readiness marker all agree, so a daemon still in its pre-validation startup window is never reported as running.
...
Its supervisor-endpoint handling (liveness, busy check, composer read, and inject submit) dispatches on the supervisor pane's backend: the tmux arm shares the same submit core used by the tmux send backend, so dim-ghost-aware and border-aware composer detection plus verified submit retry stay consistent, while the herdr arm uses the adapter's read-only pane-liveness probe, native busy state, and typed-baseline verified submit with no composer read (herdr's CLI exposes none); stalled escalation delivery raises `state/.subsuper-inject-wedged` after `FM_MAX_DEFER_SECS` instead of silently deferring forever.
