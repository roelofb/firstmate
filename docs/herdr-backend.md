...
The away-mode daemon (`bin/fm-supervise-daemon.sh`) uses the same backend submit primitive for supervisor-pane injection; on herdr its safety depends on this structural composer-state verification rather than a tmux-style cursor-row read.
