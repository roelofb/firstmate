...
| `backends/herdr.sh`      | Experimental herdr session-provider adapter used by `fm-backend.sh`; owns version/tool gating, workspace/tab creation, send, capture, native busy-state, read-only pane-liveness, current-path, label-based live discovery, and kill primitives |
...
| `fm-supervise-daemon.sh` | Presence-gated sub-supervisor for walk-away (`/afk`) supervision: wraps `fm-watch.sh`, uses the shared wake classifier and backend-aware stale rechecks, self-handles routine wakes in bash, and escalates only captain-relevant events as one verified, batched, single-line digest prefixed with a sentinel marker; validates and injects into the supervisor pane through its resolved backend (tmux or herdr), and a failed startup leaves a durable `state/.subsuper-startup-failed` marker instead of dying silently |
| `fm-afk-arm.sh`          | Verified per-home arm of the away-mode daemon; reports `started`, `healthy`, or `FAILED` (stderr captured to `state/.supervise-daemon.err`), stays attached so a later daemon death re-notifies firstmate, and `--stop` signals only this home's daemon |
